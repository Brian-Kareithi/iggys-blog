    ...
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-white/10 border-b border-white/20">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-5 sm:px-8 py-3">
          ...
          <Link href="/categories" className="text-xs text-white/50 hover:text-white transition-colors">&larr; All Categories</Link>
        </div>
      </nav>

      <main className="relative z-20 max-w-6xl mx-auto px-5 sm:px-8 pt-28 pb-20">
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-4xl font-bold text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.5)]">{categoryName}</h1>
          <p className="text-sm text-white/60 mt-2">{posts.length} posts in this category</p>
        </div>

        {posts.length === 0 ? (
          <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-10 text-center shadow-xl">
            <p className="text-white/60 text-sm">No posts in this category yet.</p>
          </div>
        ) : (
