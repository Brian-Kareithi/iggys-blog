...
export default function AboutPage() {
  return (
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <main className="relative z-20 max-w-4xl mx-auto px-5 sm:px-8 pt-28 pb-20">
        <div className="text-center mb-12">
          <div className="w-36 h-36 rounded-full bg-gradient-to-br from-accent/40 to-accent-light/20 border-2 border-white/30 mx-auto mb-6 flex items-center justify-center shadow-2xl">
            <span className="text-5xl">\uD83D\uDC64</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.5)]">Hi, I&rsquo;m KK.</h1>
          <p className="text-sm text-white/60 mt-2 max-w-xl mx-auto">Writer, developer, and lifelong learner sharing thoughts on technology, creativity, and everyday life.</p>
        </div>

        <section className="mb-12">
          <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 sm:p-8 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-4 drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">Biography</h2>
        ...
        <section className="mb-12">
          <h2 className="text-xl font-bold text-white mb-6 drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">Experience</h2>
          <div className="space-y-4">
            {experience.map((exp) => (
              <div key={exp.period} className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-xl p-5 hover:bg-white/15 transition-all duration-300 shadow-lg">
                <div className="flex items-start justify-between mb-1">
                  <div>
                    <h3 className="text-base font-bold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]">{exp.role}</h3>
                    <p className="text-sm text-accent">{exp.company}</p>
                  </div>
                  <span className="text-xs text-white/40 shrink-0 ml-4">{exp.period}</span>
                </div>
                <p className="text-sm text-white/60 mt-2">{exp.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-12">
          <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 sm:p-8 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6 drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">Skills & Technologies</h2>
            <div className="flex flex-wrap gap-3">
              {skills.map((skill) => (
                <span
                  key={skill}
                  className="px-4 py-2 bg-white/10 border border-white/20 rounded-full text-xs text-white/70 hover:bg-white/20 hover:text-white transition-all cursor-default"
                >
                  {skill}
                </span>
              ))}
        ...
        <section className="mb-12">
          <h2 className="text-xl font-bold text-white mb-6 drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">Timeline</h2>
          <div className="relative">
            <div className="absolute left-4 top-0 bottom-0 w-px bg-white/20" />
            <div className="space-y-6">
              {milestones.map((m) => (
                <div key={m.year} className="relative pl-10">
                  <div className="absolute left-2.5 top-1.5 w-3 h-3 rounded-full bg-accent border-2 border-white/20" />
                  <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-xl p-4 hover:bg-white/15 transition-all duration-300 shadow-lg">
                    <span className="text-xs font-bold text-accent">{m.year}</span>
                    <h3 className="text-sm font-bold text-white mt-1 drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]">{m.title}</h3>
        ...
        <section className="text-center">
          <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-8 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-4 drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">Let&rsquo;s Connect</h2>
            <p className="text-sm text-white/60 mb-6">Follow me on social media or drop me a message.</p>
            <div className="flex justify-center gap-4">
              {["Twitter", "GitHub", "LinkedIn", "Instagram"].map((platform) => (
                <Link
                  key={platform}
                  href="#"
                  className="w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white/60 hover:bg-white/20 hover:text-white hover:border-white/40 transition-all shadow-lg text-sm font-bold"
                >
                  {platform[0]}
                </Link>
              ))}
