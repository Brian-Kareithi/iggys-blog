      ...
      <header className="flex items-center justify-between px-6 sm:px-10 py-3 sm:py-4 bg-white border-b border-gray-200 shadow-sm">
        ...
        <div className="flex items-center gap-4 sm:gap-6">
          <div className="hidden sm:block relative">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground/40 pointer-events-none">
            ...
            <input
              ...
              className="w-56 lg:w-64 rounded-xl border border-gray-200 bg-white pl-9 pr-4 py-2 text-sm text-foreground placeholder-foreground/40 outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all duration-300"
            />
          </div>
          <div className="w-9 h-9 rounded-full bg-accent flex items-center justify-center text-white text-xs font-bold cursor-pointer shadow-lg hover:bg-accent/90 transition-all duration-300">
            IB
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-6 sm:px-10 py-6 sm:py-8 overflow-y-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-[12px] text-foreground/40 mt-0.5">{filteredPosts.length} posts total</p>
          </div>
          <button
            onClick={openNewPost}
            className="group flex items-center gap-2 bg-white border border-gray-200 text-foreground text-sm font-medium tracking-widest px-6 py-2.5 rounded-xl hover:bg-accent-light hover:border-gray-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 shadow-sm w-full sm:w-auto justify-center"
          >
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          {filteredPosts.length === 0 ? (
            <div className="px-6 py-12 text-center text-foreground/40 text-sm">
            ...
            filteredPosts.map((post, idx) => (
              ...
              >
                <div className="w-[35%] sm:w-[38%] font-medium text-foreground truncate pr-2 text-xs sm:text-sm flex items-center gap-2.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-accent/60 shrink-0" />
                  {post.title}
                </div>
                <div className="w-[18%] sm:w-[15%]">
                  {post.status === "published" ? (
                    <span className="inline-flex items-center gap-1 px-2 sm:px-2.5 py-0.5 rounded-full bg-accent text-white text-[10px] font-medium shadow-sm border border-accent/50">
                      <span className="w-1 h-1 rounded-full bg-white" />
                      Pub.
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 sm:px-2.5 py-0.5 rounded-full bg-accent-light/50 border border-gray-200 text-foreground/60 text-[10px] font-medium">
                      <span className="w-1 h-1 rounded-full bg-foreground/30" />
                      Draft
                    </span>
                  )}
                </div>
                <div className="hidden sm:block w-[15%] text-foreground/50 text-xs">{post.category}</div>
                <div className="hidden md:block w-[12%] text-foreground/50 text-xs font-medium">{post.views?.toLocaleString()}</div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/60 p-4">
          <div className="relative w-full max-w-[600px] bg-white border border-gray-200 rounded-3xl shadow-xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <label className="block text-[11px] font-bold tracking-wider text-foreground/60 mb-1.5">TITLE</label>
            <input type="text" placeholder="Post title..." value={formTitle} onChange={(e) => setFormTitle(e.target.value)} className="w-full border border-gray-200 bg-white rounded-xl px-4 py-3 text-sm text-foreground placeholder-foreground/30 outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all duration-300 mb-5" />
            <label className="block text-[11px] font-bold tracking-wider text-foreground/60 mb-1.5">CATEGORY</label>
            <select value={formCategory} onChange={(e) => setFormCategory(e.target.value as Category)} className="w-full border border-gray-200 bg-white rounded-xl px-4 py-3 text-sm text-foreground outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-all duration-300 mb-5 appearance-none">
              {["Travel", "Style", "Tech"].map((c) => (<option key={c} value={c} className="bg-white text-foreground">{c}</option>))}
            </select>
            <label className="block text-[11px] font-bold tracking-wider text-foreground/60 mb-1.5">CONTENT</label>
            <div className="border border-gray-200 bg-white rounded-xl mb-5 overflow-hidden">
              <div className="flex items-center gap-1.5 px-3 py-2 border-b border-gray-100 bg-accent-light/50">
                {[{ title: "Bold", d: "M15.6 10.79c.97-.67 1.65-1.77 1.65-2.79 0-2.26-1.75-4-4-4H7v14h7.04c2.09 0 3.71-1.7 3.71-3.79 0-1.52-.86-2.82-2.15-3.42zM10 6.5h3c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-3v-3zm3.5 9H10v-3h3.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5z" }, { title: "Italic", d: "M10 4v3h2.21l-3.42 8H6v3h8v-3h-2.21l3.42-8H18V4z" }].map((icon) => (
                  <button key={icon.title} className="p-1.5 rounded-lg text-accent hover:bg-accent-light transition-colors" title={icon.title}><svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d={icon.d} /></svg></button>
                ))}
                ...
                {[{ title: "Link", d: "M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71", d2: "M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" }, { title: "Quote", d: "M6 17h3l2-4V7H5v6h3zm8 0h3l2-4V7h-6v6h3z" }].map((icon) => (
                  <button key={icon.title} className="p-1.5 rounded-lg text-accent hover:bg-accent-light transition-colors" title={icon.title}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{icon.d2 ? <><path d={icon.d} /><path d={icon.d2} /></> : <path d={icon.d} />}</svg></button>
                ))}
              </div>
              <textarea placeholder="Write your post content here..." value={formContent} onChange={(e) => setFormContent(e.target.value)} rows={6} className="w-full px-4 py-3 text-sm text-foreground placeholder-foreground/30 outline-none resize-none bg-transparent" />
            </div>
            <label className="block text-[11px] font-bold tracking-wider text-foreground/60 mb-1.5">FEATURED IMAGE</label>
            <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); const file = e.dataTransfer.files?.[0]; if (file) setImagePreview(URL.createObjectURL(file)); }}
              className={`border-2 border-dashed rounded-2xl p-6 sm:p-8 text-center cursor-pointer transition-all duration-300 mb-6 ${dragOver ? "border-accent bg-accent/15" : "border-gray-200 hover:border-accent/60 bg-accent-light/50 hover:bg-accent-light"}`}
            ...
            >
              {imagePreview ? (
                ...
                <div className="text-foreground/50">
                  ...
                  <p className="text-[11px] text-foreground/30 mt-1">PNG, JPG, WebP up to 5MB</p>
                </div>
              )}
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 pt-2">
              <div className="flex items-center gap-3">
                <span className="text-[11px] font-bold tracking-wider text-foreground/60">STATUS</span>
                <button onClick={() => setFormStatus(formStatus === "draft" ? "published" : "draft")}
                  className={`relative w-12 h-6 rounded-full transition-all duration-300 ${formStatus === "published" ? "bg-accent shadow-md shadow-accent/30" : "bg-gray-200"}`}>
                ...
                <span className="text-xs text-foreground/60 font-medium">{formStatus === "published" ? "Published" : "Draft"}</span>
              </div>
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button onClick={() => { setFormStatus("draft"); handleSave(); }} className="flex-1 sm:flex-initial px-5 py-2.5 border border-gray-200 bg-white text-foreground text-xs font-medium tracking-wider rounded-xl hover:bg-accent-light hover:border-gray-300 transition-all duration-300">SAVE DRAFT</button>
